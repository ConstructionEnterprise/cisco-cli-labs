...
export function isCiscoCommand(command: string, mode: Mode): boolean {
  ...
  if (mode === "privileged" && (command === "show hosts" || command === "show ip ospf database" || /^nslookup \S+$/.test(command))) return true;
  ...
  if (mode === "privileged") {
    return /^(show (access-lists(?: \S+)?|arp|cdp neighbors(?: detail)?|clock|controllers|crypto key mypubkey rsa|etherchannel summary|interfaces(?: (?:g\d+\/\d+(?:\.\d+)?(?: switchport)?|gigabitethernet ?\d+\/\d+(?: switchport)?|port-channel ?\d+(?: switchport)?|status|trunk|counters errors|switchport))?|ip (?:dhcp (?:binding|conflict|pool|server statistics)|interface(?: brief| \S+)?|mroute|nat (?:translations(?: verbose)?|statistics)|ospf(?: neighbor| interface(?: brief| \S+)?)|protocols|route(?: (?:connected|static|ospf|\S+))?)|ipv6 (?:interface(?: brief| \S+)?|mroute|neighbors|ospf(?: neighbor| interface)|route(?: (?:connected|static|ospf|\S+))?|pim (?:neighbor|interface))|mac address-table(?: (?:dynamic|secure|interface \S+))?|port-security(?: interface \S+)?|running-config(?: (?:interface \S+|\| section (?:interface|ipv6|ip route|nat|access-list|dhcp|router ospf|username|line vty|crypto|multicast)))?|spanning-tree(?: (?:vlan \d+|root|summary|interface \S+ detail))?|startup-config|users|version|vlan brief|ip ssh|login)|ping (?:[0-9a-f:.]+|\S+)|traceroute [0-9a-f:.]+|copy running-config startup-config|write memory|terminal (?:length \d+|monitor)|clear (?:counters|ip route|ipv6 route))$/.test(command);
  }

  if (mode === "config") {
    return /^(hostname [a-z0-9][a-z0-9.-]{0,62}|enable secret \S+|no ip domain-lookup|ip domain-name \S+|crypto key generate rsa modulus \d+|ip multicast-routing|ipv6 unicast-routing|ipv6 multicast-routing|vlan (?:[1-9]\d{0,3})|interface (?:g\d+\/\d+(?:\.\d+)?|gigabitethernet ?\d+\/\d+|loopback \d+|vlan \d+|port-channel ?\d+)|interface range .+|router ospf \d+|ipv6 router ospf \d+|ip dhcp (?:excluded-address [0-9.]+(?: [0-9.]+)?|pool \S+)|line (?:vty \d+(?: \d+)?|console \d+)|ip access-list (?:standard|extended) \S+|access-list \d+ (?:permit|deny) .+|ip nat inside source list \d+ interface g\d+\/\d+ overload|spanning-tree (?:mode (?:rapid-pvst|pvst)|vlan \d+ (?:root (?:primary|secondary)|priority \d+))|username \S+ (?:privilege \d+ )?secret \S+|ip route [0-9.]+ [0-9.]+ [0-9.]+|ipv6 route [0-9a-f:]+\/\d+ [0-9a-f:]+|ip default-gateway [0-9.]+)$/.test(command);
  }
