...
* **Live verification tables:** A dedicated panel below the IOS simulator exposes the active device's IP interface, IP address, default gateway, ARP, MAC address, trunk, EtherChannel, ACL, and NAT/PAT state while the learner builds that state through the CLI. Tables are modeled from the current session and show honest empty states when no entries have been learned or configured.
