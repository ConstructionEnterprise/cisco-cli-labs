...
* **Live verification tables:** A dedicated panel below the IOS simulator exposes the active device's IP interface, ARP, MAC address, trunk, and EtherChannel state while the learner builds that state through the CLI. Tables are modeled from the current session and show honest empty states when no entries have been learned or configured.
