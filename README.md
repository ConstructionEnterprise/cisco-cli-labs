...
* **Device MAC identity inventory:** Every device in the selected lab receives a deterministic unique MAC identity at initialization. The **Device MACs** verification tab opens by default and lists every router, Layer 3 switch, Layer 2 switch, firewall, virtual router, server, and PC in the topology before any CLI command or traffic is run. The separate **MAC Address** tab remains the learned forwarding database for modeled Layer 2 traffic.
